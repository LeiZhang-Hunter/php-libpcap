...
{
    bzero(NG(pcap_lib),sizeof(pcap_module));
    NG(pcap_lib)->max_packet_num = 0;
    bzero(NG(pcap_lib)->err_buf,sizeof(pcap_lib->err_buf));
    NG(pcap_lib)->pcap_config_check = pcap_config_check;
    NG(pcap_lib)->pcap_if_t_to_zend_hash = pcap_if_t_to_zend_hash;
    NG(pcap_lib)->find_all_devs = pcap_find_all_devs;
    NG(pcap_lib)->free_all_devs = pcap_free_all_devs;
}
...
{
    pcap_if_t *alldevs;
    if(pcap_findalldevs(&alldevs,NG(pcap_lib)->err_buf) == PCAP_FALSE)
    {
        return NULL;
    }
...
PCAP_BOOL pcap_config_check(zval* config)
{
    ...
    NG(pcap_lib)->dev_name = Z_STR(*result);
    ...
    if(EXPECTED(!(result = zend_hash_str_find(config_table,PCAP_MAXPACKET_NUM,strlen(PCAP_MAXPACKET_NUM)))))
    {
        ...
        NG(pcap_lib)->max_packet_num = Z_LVAL(*result);
    }
